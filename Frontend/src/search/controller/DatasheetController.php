		...
		if (!empty($object)&&!empty($message)&&filter_var($sendermail, FILTER_VALIDATE_EMAIL)) {
			...
			foreach ($response['_source']['INTRO']['FILE_CREATOR'] as $key => $value) {
				if ($author_name==$value["NAME"]&&$author_firstname==$value["FIRST_NAME"]) {
					$mail=$value["MAIL"];
					mail("<".$mail.">", 'Contact from ORDaR :'.$object, $sendermail." want to know something about this dataset: <br>DOI: ".$doi."<br>Title: ".$title." <br> Message from ".$sendermail.": <br> ".$message, "From:<noreply@ordar.otelo.univ-lorraine.fr>");
				}
			}
			if ($mail==true) {
				return $error="false";
