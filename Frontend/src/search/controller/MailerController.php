...
class MailerController
{
    ...
    public function Send_Mail_author($doi, $response, $author_name, $author_firstname, $object, $message, $sendermail)
    {
     ...
     * @return true if error, else false
     */
    public function Send_Mail_To_uploader($authors, $title, $doi, $description)
    {
        $connected = self::CheckSMTPstatus();
        if ($connected === true) {
            $file    = new File();
    ...
    public function Warning_mail($filename)
    {
        $connected = self::CheckSMTPstatus();
        if ($connected === true) {
            $file   = new File();
    ...
    public function Warning_mail_bad_path_data()
    {
        $connected = self::CheckSMTPstatus();
        if ($connected === true) {
            $file   = new File();
